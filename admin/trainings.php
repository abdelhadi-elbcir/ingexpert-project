...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($action === "create" || $action === "update")) {
  ...
  if ($duration === "") $errors[] = "La durée/date affichée est obligatoire.";
  if ($description === "") $errors[] = "La description est obligatoire.";
  if ($link_url === "") $errors[] = "Le lien est obligatoire.";
...
<div class="admin-shell">
  ...
  <aside class="admin-sidebar">
    ...
    <nav class="admin-nav" aria-label="Navigation admin">
      <a href="messages.php"><span class="nav-dot" aria-hidden="true"></span>Messages</a>
    ...
    <header class="admin-topbar">
      <div class="admin-topbar-inner">
        ...
        <div class="admin-actions">
          <span class="badge"><span class="badge-dot" aria-hidden="true"></span><?= count($trainings) ?> entrée(s)</span>
          ...
          <form method="post" action="trainings.php<?= $editItem ? "?action=edit&id=".(int)$editItem["id"] : "" ?>">
            ...
            <div class="grid" style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
              ...
              <div class="field">
                ...
                <input type="text" name="duration" value="<?= e((string)($editItem["duration"] ?? "")) ?>" placeholder='Ex: "3 jours | Du 22 au 24 sept 2025"' required>
              </div>

              <div class="field" style="grid-column:1 / -1;">
                <label>Description (courte)</label>
                <input type="text" name="description" value="<?= e((string)($editItem["description"] ?? "")) ?>" required>
              </div>

              <div class="field" style="grid-column:1 / -1;">
                <label>Lien (URL)</label>
                <input type="text" name="link_url" value="<?= e((string)($editItem["link_url"] ?? "")) ?>" placeholder="/ingexpert/..." required>
              </div>

              <div class="field">
                <label>Date début (optionnel)</label>
                <input type="text" name="start_date" value="<?= e((string)($editItem["start_date"] ?? "")) ?>" placeholder="YYYY-MM-DD">
              </div>

              <div class="field">
                <label>Date fin (optionnel)</label>
                <input type="text" name="end_date" value="<?= e((string)($editItem["end_date"] ?? "")) ?>" placeholder="YYYY-MM-DD">
              </div>
            ...
            <table class="table" style="min-width:1050px;">
              ...
              <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <div style="display:flex; gap:8px; justify-content:flex-end;">
                        <a class="btn btn-ghost btn-sm" href="trainings.php?action=edit&id=<?= (int)$t["id"] ?>">Modifier</a>

                        <form method="post" action="trainings.php" onsubmit="return confirm('Supprimer cette formation ?');">
                ...
                <?php if (empty($trainings)): ?>
                  <tr><td colspan="9" class="muted" style="padding:18px;">Aucune formation.</td></tr>
                <?php endif; ?>
              </tbody>
            </table>
          </div>
